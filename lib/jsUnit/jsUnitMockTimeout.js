 ...
 * Modified by RRRM. 
 * 
 * - added enableMocks/disableMocks methods to allow restore the original setTimeout/setInterval, 
 *   clearTimeout/clearInterval functions 
 * 
 */
...
Clock.enableMocks = function () {
  //save the original functions
  Clock.originalSetTimeout = window.setTimeout;
  Clock.originalClearTimeout = window.clearTimeout;
  Clock.originalSetInterval = window.setInterval;
  Clock.originalClearInterval = window.clearInterval;
  //pointers to the mocked functions
...
Clock.disableMocks = function () {
  //restore the original functions
  window.setTimeout = Clock.originalSetTimeout;
  window.clearTimeout = Clock.originalClearTimeout;
 ...
 */
function setTimeoutMock(funcToCall, millis) {
 ...
 * @return the ID of the timeout, which is the index of the scheduledFunction in Clock's list of scheduledFunctions
 */
function setIntervalMock(funcToCall, millis) {
    Clock.timeoutsMade = Clock.timeoutsMade + 1;
 ...
 */
function clearTimeoutMock(timeoutKey) {
    Clock.scheduledFunctions[timeoutKey] = undefined;
}
 ...
 */
function clearIntervalMock(timeoutKey) {
    Clock.scheduledFunctions[timeoutKey] = undefined;
}